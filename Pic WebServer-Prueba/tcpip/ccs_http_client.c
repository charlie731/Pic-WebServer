...
{
   debug_httpc(debug_putc, "HttpClientSetResponseBuffer(max=%lu)\r\n", maxSize);
   
   g_HttpClient.response = response;
   g_HttpClient.responseSize = maxSize - 1;  //save one space for null
}
         ...
         if (c == '\n')
         {
            ...
            else 
            {
               ...
               if (p)
               {
                  ...
                  p += 16;
                  g_HttpClient.contentLen = atol(p);
...
void HttpClientTask(void)
{
   ...
   {
      debug_httpc(debug_putc, "HttpClientTask() %X->%X rem:%LU\r\n", debug, g_HttpClient.sm, HttpClientIsGetReady());
   ...
   switch(g_HttpClient.sm)
   {
      ...
      case HTTP_CLIENT_SM_CONNECTING:
         ...
         else if ((TickGet() - g_HttpClient.t) >= ((TICK)TICKS_PER_SECOND*30))
         {
      ...
      case HTTP_CLIENT_SM_PUT_GET:
         ...
         if (avail >= 16)
         {
            ...
            else
            {
               ...
               HTTP_CLIENT_TASK_STATE(HTTP_CLIENT_SM_PUT_HEADERS);
               cont = TRUE;
            }
         }
         break;

      case HTTP_CLIENT_SM_PUT_HEADERS:
         avail = HttpClientIsPutReady();
         if (avail >= 32)
         {
            if (g_HttpClient.idx == 0)
      ...
      case HTTP_CLIENT_SM_GET_BODY:
         if (g_HttpClient.response)
         {
            ...
            if (avail)
            {
               HttpClientGetArray(&g_HttpClient.response[g_HttpClient.idx], avail);
               g_HttpClient.idx += avail;
               g_HttpClient.response[g_HttpClient.idx] = 0;
            }
         }
         if 
         (
         ...
         {
            debug_httpc(debug_putc, "HttpClientTask() DONE\r\n");
            HTTP_CLIENT_TASK_DONE(g_HttpClient.ec);
         }
         break;
   }
