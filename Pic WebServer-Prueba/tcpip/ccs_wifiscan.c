...
enum
{
  ...
  #if defined(EE_LOC_WIFI_SCAN_RESULTS) && !defined(CCS_WIFISCAN_IMMEDIATE_READ)
...
#if defined(EE_LOC_WIFI_SCAN_RESULTS)
#if (EE_SIZE_PER_NUM_WIFI_SCAN_RESULTS < sizeof(tWFScanResult))
...
/*
   ...
   (
     #if !defined(EE_LOC_WIFI_SCAN_RESULTS)
      (g_WifiScanApNum != 0xFF)
     #elif defined(EE_LOC_WIFI_SCAN_RESULTS) && !defined(CCS_WIFISCAN_IMMEDIATE_READ)
      (g_WifiScanState == WIFI_SCAN_SAVING_AP_TO_EE)
     #else
      FALSE
     #endif
...
{  
   ...
   else if (g_WifiScanState == WIFI_SCAN_DOING_AP)
   {
     ...
     #if defined(EE_LOC_WIFI_SCAN_RESULTS) && defined(CCS_WIFISCAN_IMMEDIATE_READ)
      _WIFIScanResultHandleAP();
     #elif defined(EE_LOC_WIFI_SCAN_RESULTS)
      g_WifiScanState = WIFI_SCAN_SAVING_AP_TO_EE;
      g_WifiScanToEE = num;
     #else
      g_WifiScanState = WIFI_SCAN_STATE_IDLE;
