...
MPFS _g_HttpMpfsStart[HTTP_NUM_SOCKETS];
#if (HTTP_NUM_SOCKETS == 1)
   #define g_HttpMpfsStart(x) _g_HttpMpfsStart[0]
#else
   #define g_HttpMpfsStart(x) _g_HttpMpfsStart[x]
#endif
...
{
   debug_html(debug_html_putc, "%c", c);   
...
MIME_T HTTPFindMimeType(char *fname)
{
   ...
   for (i=0; i<HTTP_NUM_MIME_TYPES; i++)
   {
      ...
      debug_html(debug_html_putc, "\r\nHTML Ext check %u '%s' '%s' vs '%s'(0x%LX) [%X]", i, fname, extension, str, &gc_MimeTypes[i][0], stricmp(str, extension));
...
char TCPPutFileParseConst(MPFS *fstream, unsigned int16 max, int1 isPix)
{
   ...
   */
   ...
   while (max && !escape && !MPFSIsEOF())
   {
      ...
      readCount = (max < HTTP_GET_PARAM_MAX_SIZE ? max : HTTP_GET_PARAM_MAX_SIZE);
...
TCP_PUT_CONST_EC TCPPutFileConstChunk(MPFS *retAddy, int1 isPix)
{
   ...
   debug_html(debug_html_putc, "TCPPutFileConstChunk(%lX,%u) START\r\n", *retAddy, isPix);
   ...
   debug_html(debug_html_putc, "TCPPutFileConstChunk() END %LX\r\n", *retAddy);
...
int1 tcp_http_put_file(unsigned int16 errorCode, MPFS* MMCfile, char* fname)
{
   ...
   position = MPFSTell()-g_HttpMpfsStart(g_HttpCurrConn);
...
//                      fprintf(USER,"HTTP %U PVALUE: %s FILE OPENED: %s, handle:%LX\r\n", hs, pValue, page, fstream);
                     ...
                     else if (http_cmd[g_HttpCurrConn] != HTTP_REQ_UNKOWN)   //we processed a GET or POST
                     {
                        ...
                        if (strcmp(pKey, http_len_str)==0)
                           http_post_len[g_HttpCurrConn]=strtoul(pValue, NULL, 10);
                      #if HTTP_LAST_MODIFIED_CHECK
...
//            fprintf(USER,"HTTP %U SEND RESPONSE\r\n", hs);
            ...
            debug_html(debug_html_putc, "MPFSOpen('%s')=%LX\r\n", page, fstream);
...
//                  fprintf(USER,"HTTP %U RESPONSE SENT\r\n", hs);
      ...
      {
         debug_html(debug_html_putc, "\r\nHTML %U %X->%X\r\n", g_HttpCurrConn, debug, http_state[g_HttpCurrConn]);
