 ...
 ********************************************************************/
/*
   Darren Rook(CCS) - Moved the SSL.H and HTTP2.H includes before the
      MAC.H, otherwise MAC.H was confused because the RESERVED_HTTP_MEMORY
      and RESERVED_SSL_MEMORY weren't defined yet.
*/
...
#define TCPIP_STACK_VERSION       "v5.42"      // TCP/IP stack version
   ...
   #define STACK_EXT_MODULE_HAS_TCP_UDP
   #define STACK_EXT_MODULE_HAS_DNSS //it has a DNS server but the DNSS_ONLY_URL option doesn't work
#endif
 ...
 *******************************************************************/
   ...
   #if (defined(STACK_USE_HTTP2_SERVER) || defined(STACK_USE_FTP_SERVER)) && !defined(STACK_USE_MPFS2)
   ...
   #if defined(STACK_USE_SNMP_SERVER) && !defined(STACK_USE_MPFS2)
      #define STACK_USE_MPFS2
   #endif
   ...
   // Include modules required by specific HTTP demos
      ...
      #ifdef STACK_USE_HTTP_EMAIL_DEMO
...
#if (defined(STACK_USE_ICMP_SERVER) || defined(STACK_USE_ICMP_CLIENT)) && !defined(STACK_EXT_MODULE_HAS_TCP_UDP)
   #include "ICMP.h"
#endif
