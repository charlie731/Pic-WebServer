...
#ifndef debug_mpfs_array
   #define debug_mpfs_array(p, n)
#endif
   ...
   _currentFile = MPFS_INVALID;
    ...
    for(;;)
    {
        // Bring current FAT entry into RAM.
         ...
         #endif
      #endif
       debug_mpfs_array(&entry, sizeof(entry));
...
#if defined(__C30__) && !defined(MPFS_USE_EEPROM) && !defined(__PCD__)
/*********************************************************************
 ...
 * Note:            None.
 ********************************************************************/
 /* darren commenting this function out since it's broken.  
 uses a pointer initialized by mpfsopen(), which means you can't use this to
 have multiple files open at once.  you also can't use this after using
 MPFSGetBegin() to continue from a previous spot in the file
