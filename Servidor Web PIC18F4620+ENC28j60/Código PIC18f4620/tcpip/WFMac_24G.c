...
/* used for assertions */
#if defined(WF_DEBUG)
   #ifdef WF_MODULE_NUMBER
   #undef WF_MODULE_NUMBER
   #endif
...
void MACInit(void)
{
    ...
    SetLogicalConnectionState(FALSE);  //darren added
}
...
void MACMemCopyAsync(PTR_BASE destAddr, PTR_BASE sourceAddr, WORD len)
{
    ...
    //UINT8 copyBuf[8];
      ...
      #define MACMemCopyAsync_copyBuf  GENERIC_SCRATCH_BUFFER
   #else
      UINT8 MACMemCopyAsync_copyBuf[8];
      #warning !! USING SMALL COPY BUFFER, CAN MAKE TCPFLUSH() SLOW
   #endif
    UINT16 writeIndex, readIndex;
    UINT16 bytesLeft;
    UINT16 origRawIndex;

  #if defined(__DEBUG_HTTP5__)
   TICK t;
   t = TickGet();
  #endif
   ...
   #if defined(__PCD_QUIRK3__)
      #warning !! QUIRK MODE ENABLED
    if(bit_test(destAddr, 15))
   #else
  ...
  #if defined(__DEBUG_HTTP5__)
   debug_printf(debug_putc, "MACC0 %LX\r\n", TickGet() - t);
   t = TickGet();
  #endif

    /* if using current location of read pointer */
   #if defined(__PCD_QUIRK3__)
    if(bit_test(sourceAddr, 15))
   #else
  ...
  #if defined(__DEBUG_HTTP5__)
   debug_printf(debug_putc, "MACC1 %LX\r\n", TickGet() - t);
   t = TickGet();
  #endif    
    ...
    if ((destAddr >= BASE_SCRATCH_ADDR/*BASE_TCB_ADDR*/) && (sourceAddr >= BASE_SCRATCH_ADDR/*BASE_TCB_ADDR*/) )
    {
           ...
           )
        {
            ...
            while (bytesLeft > 0)
            {
                ...
                if (bytesLeft >= sizeof(MACMemCopyAsync_copyBuf))
                {
                    /* read a block of bytes from the source and write to destination */
                    RawRead( rawScratchId, readIndex,  sizeof(MACMemCopyAsync_copyBuf), MACMemCopyAsync_copyBuf);
                    RawWrite(rawScratchId, writeIndex, sizeof(MACMemCopyAsync_copyBuf), MACMemCopyAsync_copyBuf);

                    // index to next block in source and dest
                    readIndex  += sizeof(MACMemCopyAsync_copyBuf);
                    writeIndex += sizeof(MACMemCopyAsync_copyBuf);
                    bytesLeft  -= sizeof(MACMemCopyAsync_copyBuf);
                }
                ...
                else
                {
                    if (bytesLeft > 0)
                    {
                        RawRead( rawScratchId, readIndex,  bytesLeft, MACMemCopyAsync_copyBuf);
                        RawWrite(rawScratchId, writeIndex, bytesLeft, MACMemCopyAsync_copyBuf);
                        bytesLeft = 0;
                    }    
                }        
            }    
        ...
        else if (destAddr > sourceAddr)
        {
            ...
            while (bytesLeft > 0)
            {
                ...
                if (bytesLeft >= sizeof(MACMemCopyAsync_copyBuf))
                {
                    RawRead(rawScratchId,  readIndex  - sizeof(MACMemCopyAsync_copyBuf) + 1, sizeof(MACMemCopyAsync_copyBuf), MACMemCopyAsync_copyBuf);
                    RawWrite(rawScratchId, writeIndex - sizeof(MACMemCopyAsync_copyBuf) + 1, sizeof(MACMemCopyAsync_copyBuf), MACMemCopyAsync_copyBuf);
                ...
                else
                {
                    if (bytesLeft > 0)
                    {
                        RawRead(rawScratchId, readIndex - bytesLeft + 1, bytesLeft - 1, MACMemCopyAsync_copyBuf);
                        RawWrite(rawScratchId, writeIndex - bytesLeft + 1, bytesLeft - 1, MACMemCopyAsync_copyBuf);
                        bytesLeft = 0;
                    }    
                }        
            } // end while    
  ...
  #if defined(__DEBUG_HTTP5__)
   debug_printf(debug_putc, "MACC2 %LX\r\n", TickGet() - t);
   t = TickGet();
  #endif
  ...
  #if defined(__DEBUG_HTTP5__)
   debug_printf(debug_putc, "MACC3 %LX\r\n", TickGet() - t);
   t = TickGet();
  #endif
  ...
  #if defined(__DEBUG_HTTP5__)
   debug_printf(debug_putc, "MACC4 %LX\r\n", TickGet() - t);
   t = TickGet();
  #endif
