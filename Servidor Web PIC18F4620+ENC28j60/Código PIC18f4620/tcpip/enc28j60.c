...
void MACFlush(void)
{
   ...
   if(ReadETHReg(EIR).EIRbits.TXERIF)
   {
      ...
      BFCReg(ECON1, EIR_TXERIF); // Cambiado por Ver error http://forum.mysensors.org/topic/536/problems-with-enc28j60-losing-connection-freezing-using-uipethernet-or-ethershield-read-this
   }
   
   BFCReg(EIR, EIR_TXERIF | EIR_TXIF); // Ver _ Dejar solo esto y arriba borrar _ VEr// Cambiado por Ver error http://forum.mysensors.org/topic/536/problems-with-enc28j60-losing-connection-freezing-using-uipethernet-or-ethershield-read-this
