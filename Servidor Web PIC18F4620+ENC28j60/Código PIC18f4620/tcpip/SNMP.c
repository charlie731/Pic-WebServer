 ...
 * Darren Rook/CCS      09/23/14   Added a check to see that SNMP_NMS_PORT and
 *                                     SNMP_AGENT_PORT aren't 0 before trying
 *                                     to open them.
 *                                 Changed a few vars from 'int' to 'INT' to
 *                                     fix typedef issues.
 * Darren Rook/CCS      03/23/15 When calling UDPIsPutReady(), make sure there is
 *                                  at least 128 bytes available.  Otherwise
 *                                  I was having problems sending multiple
...
#ifndef debug_snmp
   #define debug_snmp(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t)
#endif
...
//WORD msgSecrtyParamLenOffset;  //__CCS__ __PCH__ __PCD__ not sure why this was declared twice
...
void _SNMPDuplexInit(UDP_SOCKET socket)
{
   ...
   while( UDPIsPutReady(socket) < 128) ;
...
void SNMPInit(void)
{
   debug_snmp(debug_putc, "SNMPInit()\r\n");
...
BOOL SNMPIsNotifyReady(IP_ADDR* remoteHost)
{
   ...
   if ((SNMPNotifyInfo.socket == INVALID_UDP_SOCKET) && SNMP_NMS_PORT)
   {
...
BOOL SNMPNotify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index)
{
   ...
   if ( hMPFS == MPFS_INVALID_HANDLE )
   {
      ...
      debug_snmp(debug_putc, "SNMPNotify() ERR_MPFS\r\n");
      return FALSE;
   }
   ...
   if((packetStructLenOffset == 0)&&(pduStructLenOffset==0))
   {
      ...
      if ( !GetOIDStringByID(SNMPNotifyInfo.trapIDVar, &rec, OIDValue, &OIDLen) )
      {
         MPFSClose(hMPFS);
         UDPClose(SNMPNotifyInfo.socket);
         debug_snmp(debug_putc, "SNMPNotify() ERR_OID1\r\n");
         return FALSE;
      }
      _SNMPPut(ASN_OID);
      agentIDLen = OIDLen;
   ...
   if(gGenericTrapNotification != ENTERPRISE_SPECIFIC)
   {
      ...
      if(pOIDValue == NULL)
      {
         MPFSClose(hMPFS);
         UDPClose(SNMPNotifyInfo.socket);
         debug_snmp(debug_putc, "SNMPNotify() ERR_OID2\r\n");
         return FALSE;
      }
      // Copy OID string into PDU.
      _SNMPPut(ASN_OID);
      ...
      if ( !GetOIDStringByID(SNMPNotifyInfo.agentIDVar, &rec, OIDValue, &OIDLen) )
      {
         MPFSClose(hMPFS);
         UDPClose(SNMPNotifyInfo.socket);
         debug_snmp(debug_putc, "SNMPNotify() ERR_OID3\r\n");
         return FALSE;
      }
      if ( !rec.nodeInfo.Flags.bIsAgentID )
      {
         MPFSClose(hMPFS);
         UDPClose(SNMPNotifyInfo.socket);
         debug_snmp(debug_putc, "SNMPNotify() ERR_AGENT\r\n");
         return FALSE;
      }
   ...
   else
   {
      ...
      if ( !GetOIDStringByID(var, &rec, OIDValue, &OIDLen) )
      {
         MPFSClose(hMPFS);
         UDPClose(SNMPNotifyInfo.socket);
         debug_snmp(debug_putc, "SNMPNotify() ERR_OID4\r\n");
         return FALSE;
      }      
      pOIDValue = OIDValue;
      ...
      if ( !GetDataTypeInfo(rec.dataType, &dataTypeInfo) )
      {
         MPFSClose(hMPFS);
         UDPClose(SNMPNotifyInfo.socket);
         debug_snmp(debug_putc, "SNMPNotify() ERR_DATAREC\r\n");
         return FALSE;
      }
